...
class DiffusionEquationLossNet(nn.Cell):
    ...
    def get_variable(self, InSet, BdSet, g_fun_val):
        ...
        self.InSet_l = Parameter(
            Tensor(InSet.l, mstype.float32), name="InSet_l", requires_grad=False)
        self.InSet_gx = Parameter(
            Tensor(InSet_gx, mstype.float32), name="InSet_gx", requires_grad=False)
        self.InSet_lx = Parameter(
            Tensor(InSet.lx, mstype.float32), name="InSet_lx", requires_grad=False)
        self.InSet_a = Parameter(
            Tensor(InSet.a, mstype.float32), name="InSet_a", requires_grad=False)
        self.InSet_c = Parameter(
            Tensor(InSet.c, mstype.float32), name="InSet_c", requires_grad=False)
        if BdSet.has_neumann_boundary:
            self.has_neumann_boundary = True
            self.BdSet_nlength = BdSet.n_length
            self.BdSet_nr = Parameter(
                Tensor(BdSet.n_r, mstype.float32), name="BdSet_nr", requires_grad=False)
            self.BdSet_nl = Parameter(
                Tensor(BdSet.n_l, mstype.float32), name="BdSet_nl", requires_grad=False)
